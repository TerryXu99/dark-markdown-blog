...
const MOCK_AGENT_REPLIES: Record<string, string[]> = {
  default: [
    'Hey, this is ViperXu site mode.',
    'The real agent endpoint is not wired yet — this is a local mock.',
    'Try `chat what are you learning?` or `chat internship?` for canned answers.'
  ],
  building: [
    'Right now: cleaning up this blog, learning Agent development,',
    'and preparing notes/projects for future internship applications.'
  ],
  hire: [
    'Currently looking for an internship.',
    'Best public link for now: `connect` for GitHub.'
  ],
  stack: [
...
export const commands: CommandRegistry = {
      ...
      push([
        {
          ...
          node: (
            <span>
              <span className='wt-tone-primary'>ViperXu</span>
              <span className='wt-tone-muted'> · student based in Wuhan, Hubei</span>
            </span>
          )
        },
        { kind: 'text', tone: 'muted', text: '  ↳ China University of Geosciences (Wuhan)' },
        { kind: 'text', tone: 'muted', text: '  ↳ looking for an internship' },
        { kind: 'text', tone: 'muted', text: '  ↳ learning Agent development' },
        { kind: 'spacer' },
  ...
  manifest: {
    ...
    usage: 'manifest [--url]',
    run: async ({ args, push }) => {
      const url = '/.well-known/viperxu-manifest.json'
  ...
  mail: {
    ...
    run: ({ push }) => {
      push([
        { kind: 'text', tone: 'muted', text: 'email is not public yet.' },
        { kind: 'text', tone: 'muted', text: 'try `connect` for the current public link.' }
      ])
    }
  },
