 ...
 *   - `viperxu:toggle-dev` / `viperxu:enter-dev` / `viperxu:exit-dev` custom events
...
export default function DevModeHost({ fs }: { fs: FsNode }) {
  ...
  useEffect(() => {
    ...
    window.addEventListener('viperxu:toggle-dev', onToggle)
    window.addEventListener('viperxu:enter-dev', onEnter)
    window.addEventListener('viperxu:exit-dev', onExit)
    return () => {
      window.removeEventListener('viperxu:toggle-dev', onToggle)
      window.removeEventListener('viperxu:enter-dev', onEnter)
      window.removeEventListener('viperxu:exit-dev', onExit)
    }
  }, [])
